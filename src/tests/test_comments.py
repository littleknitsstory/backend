...
def token(client, django_user_model):
    ...
    res = client.post("/api/v1/sign-up/", {"email": "test01@example.com", "password": "string8euwq"}, format="json")
...
def test_post_comments(client, token):
    ...
    res = client.post("/api/v1/comments/?model_type=COMMENT&model_id=1", {"text": "test"}, format="json")
